...
export const getSaturdayColumns = (saturdayData: any[], selectedDates: Set<string>): ColumnDef[] => {
  ...
  return [
    ...
    { name: 'Present', id: '__days_present__' },
    { name: 'Absent', id: '__days_absent__' },
  ];
};
...
export const buildFridayGridColumns = (
  ...
  let translatedName = col.name;
  if (language === 'ko') {
    if (col.id === '__days_present__') translatedName = '출석 일수';
    else if (col.id === '__days_absent__') translatedName = '결석 일수';
    else if (col.id === 'first_name') translatedName = '이름';
    else if (col.id === 'last_name') translatedName = '성';
    else if (col.id === 'classroom') translatedName = '반';
    else if (col.id === 'korean_name') translatedName = '한국어 이름';
  }
  ...
  return {
    ...
    formatter: (cell: any, row: any) => {
      ...
      if (col.id === 'classroom') {
        ...
        // Replaced your fridayData reference with the 'data' parameter to ensure it's grabbing the right scope
        const taMatch = data.find(r => r.korean_name === koreanName);
        ...
        return h('select', {
          ...
          // Translates the placeholder text dynamically
